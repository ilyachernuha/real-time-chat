...
async def check_if_user_is_room_member(db: AsyncSession, user_id: uuid.UUID, room_id: uuid.UUID):
    if not await user_is_in_room(db=db, user_id=user_id, room_id=room_id):
...
async def add_users_to_room(db: AsyncSession, room: db_models.Room, user: db_models.User,
    ...
    await notification_utils.create_added_to_room_notification(db=db, user_id=user.user_id, room_id=room.room_id,
                                                               adder_id=adder_id)
