...
from . import crud, schemas, search, utils, validators, external
    ...
    users_data = [
        {
            "username": username,
            "name": user_data.name,
            "profile_picture_id": str(user_data.profile_picture_id)
        }
        for username, user_data in search.UserTrie.search(prefix=data.search, limit=10)
    ]
...
async def find_tags(sid: str, data: schemas.SearchTags):
    ...
    tags = [tag for tag in search.TagTrie.search(prefix=data.search, limit=10)]
