...
async def room_state_notification(room_id: uuid.UUID, sid: str, event: str):
...
async def add_user_to_room(user_id: uuid.UUID, room_id: uuid.UUID):
    for sid in user_sids[user_id]:
        ...
        await room_state_notification(room_id=room_id, sid=sid, event="added_to_room")


async def remove_user_from_room(user_id: uuid.UUID, room_id: uuid.UUID):
    for sid in user_sids[user_id]:
        await sio.leave_room(sid=sid, room=room_id)
        await room_state_notification(room_id=room_id, sid=sid, event="removed_from_room")
...
async def close_room(room_id: uuid.UUID, member_ids: list[uuid.UUID]):
    ...
    for user_id in member_ids:
        for sid in user_sids[user_id]:
            await room_state_notification(room_id=room_id, sid=sid, event="room_deleted")
