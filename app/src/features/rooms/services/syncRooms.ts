...
import { attachmentsCollection, db, messagesCollection, roomsCollection, usersCollection } from "@/index.native";
    ...
    await db.write(async () => {
      ...
      for (const [roomId, update] of updates) {
        ...
        for (const msg of new_messages) {
          ...
          const createAttachments = msg.attachments.map(({ attachment_id, type, presigned_url, original_name }) => {
            return attachmentsCollection.prepareCreate((a) => {
              if (original_name) a.name = original_name;
              a.type = type;
              a.server_id = attachment_id;
              a.message.id = op.id;
            });
          });
          operations.push(op, ...createAttachments);
        }
