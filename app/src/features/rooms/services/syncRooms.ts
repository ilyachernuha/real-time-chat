...
export const syncRooms = async () => {
  try {
    ...
    const existingMessagesIds = new Set(await messagesCollection.query().fetchIds());
    ...
    await db.write(async () => {
      ...
      for (const [roomId, update] of updates) {
        ...
        for (const msg of new_messages) {
          ...
          const op = messagesCollection.prepareCreate((message) => {
            message._raw.id = msg.message_id;
            message.room.id = roomId;
            message.user.id = msg.user_id;
            message.text = msg.text;
            message.timestamp = new Date(msg.created_at * 1000);
